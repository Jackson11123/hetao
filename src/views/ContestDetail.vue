<template>
  <div class="contest-detail">
    ...
    <div class="breadcrumb-area">
      <a-breadcrumb>
        <a-breadcrumb-item>
          <router-link to="/contests">比赛管理</router-link>
        </a-breadcrumb-item>
    ...
    <div v-if="activeTab === 'scoreboard'" class="tab-content">
      <div class="page-header">
        <h2>比赛成绩表</h2>
        <div class="header-actions">
          <a-button @click="handleDownloadRegistration">
            <template #icon><DownloadOutlined /></template>
            下载报名名单
          </a-button>
          <a-button type="primary" @click="handleDownloadScoreboard">
            <template #icon><DownloadOutlined /></template>
            下载实时成绩表
          </a-button>
        </div>
    ...
    <div v-if="activeTab === 'submissions'" class="tab-content">
      <div class="page-header">
        <h2>比赛全部提交</h2>
        <div class="header-actions">
          <a-button @click="handleDownloadContestSubmissions">
            <template #icon><DownloadOutlined /></template>
            下载赛中提交
          </a-button>
          <a-button type="primary" @click="handleDownloadAfterSubmissions">
            <template #icon><DownloadOutlined /></template>
            下载赛后提交
          </a-button>
        </div>
      ...
      <div class="search-area">
        <a-form layout="inline">
          ...
          <a-form-item label="语言"><a-select v-model:value="submissionSearch.language" style="width: 80px" allow-clear><a-select-option value="">全部</a-select-option></a-select></a-form-item>
          <a-form-item label="备注"><a-select v-model:value="submissionSearch.remark" style="width: 80px" allow-clear><a-select-option value="">全部</a-select-option></a-select></a-form-item>
      ...
      <div class="table-area">
        <a-table :columns="submissionColumns" :data-source="submissions" :pagination="submissionPagination" row-key="id" :scroll="{ x: 1400 }">
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'isCheating'">
              <a-checkbox :checked="record.isCheating" disabled />
            </template>
          </template>
        </a-table>
      </div>
...
const announcementColumns = [
  ...
  { title: '头图', key: 'cover', width: 100 },
  { title: '讨论标题', dataIndex: 'title', key: 'title', width: 200 },
  { title: '点赞/浏览', dataIndex: 'likeView', key: 'likeView', width: 100 },
...
const scoreboardData = ref([
  ...
  { rank: 3, username: '逆流的鱼是天生的奇', realName: '许智博', totalScore: 0 },
  { rank: 3, username: '智障老师', realName: '罗杰', totalScore: 0 }
])
const scoreboardColumns = computed(() => {
  ...
  const problemCols = problems.value.map((p, i) => ({
    title: `客观题${i + 1}`,
    key: `problem_${i + 1}`,
...
const scoreboardPagination = reactive({ current: 1, pageSize: 20, total: 4, showTotal: (total: number) => `共 ${total} 条` })

// 提交数据
const submissionSearch = reactive({ userId: '', problemOrder: '', status: '', language: '', remark: '' })
const submissions = ref([
  { id: '1', status: '55.5 Wrong Answer', problemOrder: '客观题1', problemId: 'P1511C', problemName: 'C++算法设计大赛模拟卷', userId: '21037829', username: '王艺博', time: '-', memory: '-', isCheating: false, language: 'objective', submitTime: '昨天 16:45', judgeTime: '-', remark: '赛中提交' },
  { id: '2', status: '55.5 Wrong Answer', problemOrder: '客观题1', problemId: 'P1511C', problemName: 'C++算法设计大赛模拟卷', userId: '21037829', username: '王艺博', time: '-', memory: '-', isCheating: false, language: 'objective', submitTime: '昨天 16:20', judgeTime: '-', remark: '赛中提交' },
  { id: '3', status: '85 Wrong Answer', problemOrder: '客观题1', problemId: 'P1511C', problemName: 'C++算法设计大赛模拟卷', userId: '10772032', username: '雨知非烟 (Q_Q)', time: '-', memory: '-', isCheating: false, language: 'objective', submitTime: '12月6日 21:49', judgeTime: '-', remark: '赛中提交' }
])
const submissionColumns = [
  ...
  { title: '是否作弊', key: 'isCheating', width: 80 },
  { title: '语言', dataIndex: 'language', key: 'language', width: 80 },
  { title: '提交时间', dataIndex: 'submitTime', key: 'submitTime', width: 120 },
  { title: '判题时间', dataIndex: 'judgeTime', key: 'judgeTime', width: 100 },
  { title: '备注', dataIndex: 'remark', key: 'remark', width: 100 }
]
const submissionPagination = reactive({ current: 1, pageSize: 20, total: 3 })
...
const searchPagination = reactive({ current: 1, pageSize: 10, total: 41403, showTotal: (total: number) => `共 ${total} 条` })
...
const handleAddProblem = (problem: any) => {
  const exists = problems.value.find(p => p.id === problem.id)
  if (exists) { message.warning('该题目已存在'); return }
...
const handleBatchReferenceOk = () => {
  ...
  ids.forEach(id => {
    if (!problems.value.find(p => p.id === id)) {
      problems.value.push({ id, name: `题目 ${id}`, difficulty: '入门', tags: [], createdAt: new Date().toLocaleString() })
    }
  })
...
const handleCreateAnnouncement = () => {
  router.push({ name: 'CreateContestAnnouncement', params: { id: contestId.value } })
}
...
const handlePrevPage = () => { message.info('上一页（原型展示）') }
const handleNextPage = () => { message.info('下一页（原型展示）') }
...
.pagination-nav { display: flex; justify-content: center; gap: 16px; margin-top: 16px; }
...
.search-reference-content { max-height: 500px; }
</style>
